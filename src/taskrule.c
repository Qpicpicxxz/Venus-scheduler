...
void task_bind(actor_t *g_in, actor_t *g_out, block_f *n_block,
               Taskfunc task_addr) {
  actor_t *prev_actor = g_in;
  actor_t *next_actor = g_out;
...
// task1: in[0]  --> out[0]
void task1(actor_t *g_in, actor_t *g_out) {
  // check the firing condition && catch idle block
  if (fifo_size(g_in->in[0]) >= g_in->dep_num[0] && fifo_size(&q_block) >= 1) {
    block_f *n_block = (block_f *)get_ptr(&q_block);
    printf("\nTASK1: Firing...\n");
    printf("SCHEDULER: 0x%x block is ready...\n", n_block);
    _set_block_flag(n_block, BLOCK_BUSY);
    dma_code(n_block->spm_addr, g_in->task_addr, g_in->task_len);
    data_t data = get_data(g_in->in[0]);
...
void task2(actor_t *g_in, actor_t *g_out) {
...
void task3(actor_t *g_in, actor_t *g_out) {
  if (fifo_size(g_in->in[0]) >= g_in->dep_num[0] && fifo_size(&q_block) >= 1) {
    block_f *n_block = (block_f *)get_ptr(&q_block);
    printf("\nTASK3: Firing...\n");
    printf("SCHEDULER: 0x%x block is ready...\n", n_block);
